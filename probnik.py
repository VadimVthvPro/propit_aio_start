...
def db_table_val(user_id: int , user_age: int,   user_sex: str, user_weight: float, date:str, user_aim:str, imt:float, imt_str:str, cal:float,  user_height:int ):
  cursor.execute('INSERT INTO users (user_id, user_age,  user_sex, user_weight, date, user_aim, imt, imt_str, cal, user_height) VALUES (?, ?, ?, ?, ?, ?, ?, ?,  ?, ?)', (user_id,  user_age,user_sex, user_weight, date, user_aim, imt, imt_str, cal, user_height))
  conn.commit()
...
def message_input_step(message):
  ...
  cursor.execute(f'UPDATE users SET user_height = {height} WHERE user_id = ? AND date = ?', ( message.from_user.id, datetime.datetime.now().strftime('%Y-%m-%d')))
  conn.commit()
...
def choise_of_age(message):
  cursor.execute(f"UPDATE users SET user_age = ? WHERE user_id = ? AND date = ?", (int(message.text), message.from_user.id,datetime.datetime.now().strftime('%Y-%m-%d')))
  conn.commit()
...
def choise_of_sex(message):
  cursor.execute(f"UPDATE users SET user_sex = ? WHERE  user_id = ? AND date = ?", ( f'{message.text}',message.from_user.id, datetime.datetime.now().strftime('%Y-%m-%d')))
  conn.commit()
...
def choise_of_mass(message):
  ...
  cursor.execute("SELECT user_height, user_sex, user_age FROM users WHERE user_id = ? AND date = ?",(message.from_user.id, datetime.datetime.now().strftime('%Y-%m-%d')))
  ...
  cursor.execute(f"UPDATE users SET user_weight = ?, imt = ?, imt_str = ? WHERE user_id = ? AND date = ?", (weight, imt, imt_using_words, message.from_user.id,datetime.datetime.now().strftime('%Y-%m-%d')))
  conn.commit()
  markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
  btn1 = 'Сброс веса'
...
def aim(message):
  aim = message.text
  cursor.execute('UPDATE users SET user_aim = ? WHERE user_id = ? AND date = ?', (aim,message.from_user.id, datetime.datetime.now().strftime('%Y-%m-%d')))
  conn.commit()
  cursor.execute("SELECT user_height, user_weight,imt, imt_str FROM users WHERE user_id = {}".format(int(message.from_user.id)))
  height, weight, imt, imt_using_words = cursor.fetchone()
  bot.send_message(message.chat.id, text = '{}, твой вес: {}, твой рост: {}, твой индекс массы тела:{}, и твой вес - это {}. Сечас ты сможешь выбрать свою цель, чтобы я смог помочь тебе с твоим персональным планом питания:'.format(message.from_user.first_name, weight, height, imt, imt_using_words))
  aim_work(message)


def aim_work(message):
  cursor.execute("SELECT user_aim, cal,user_sex, user_age, imt, user_weight, user_height FROM users WHERE date = ? AND user_id = ?",
...
в начале года твой рацион составлял {all_data[-1][0]} килокаларий , а конечное значение {all_data[0][0]} килокалорий,
...
и было выпито в среднем {total_w/len(food_months_with_data) * 300} воды каждый день.""",reply_markup=alfamarkup)
...
def new_tren(message):
  ...
  with GigaChat(credentials='YzY3ZWQ3MmMtN2ZlOC00ZGQzLWE5OGEtOTBjMjdlMGZjMDJiOjQ4NTI4MDM1LTliNjgtNGIwOS1hZjk3LTFkNjU1MDk2NDM4Ng==',verify_ssl_certs=False) as giga:
    tr = giga.chat(f"Придумай идеальный план на одну тренировку для {sex} {age} лет весом {weight}, который хотел бы сейчас {tren_aim}")
...
def start(message):
  ...
  markup.add(btn1, btn2)
  bot.send_photo(message.chat.id, photo = p, caption = 'Привет, {}! Бот PROпиташка поможет тебе вести индивидуальный расчет твоего питания и активности, опираясь на твои персональные параметры)'.format(message.from_user.first_name), reply_markup=markup)


def choiser(message):
  if message.text == 'Ввести названия блюд':
    mes = bot.send_message(message.chat.id, text = 'Перечисли через запятую названия блюд:', reply_markup=telebot.types.ReplyKeyboardRemove())
    bot.register_next_step_handler(mes,work_with_food)
  if message.text == 'Сфотографировать блюдо':
    bot.send_message(message.chat.id, text = 'Отправь фото блюда, чтобы система её распознала:', reply_markup=telebot.types.ReplyKeyboardRemove())
      ...
      with open('products.json') as f:
        ...
        for i in range(len(foods)):
          if foods[i]["name"] == name_a[m].title():
            ...
            counting_users_pit(user_id=message.from_user.id, date = datetime.datetime.now().strftime('%Y-%m-%d'), user_name_of_food= name_a[m].title(), b=b, g=g, u=u, food_cal = food_cal)
    bot.send_message(message.chat.id, text = 'Данные записаны)',reply_markup=alfamarkup)

def registr(message):
  db_table_val(user_id = message.from_user.id, date = datetime.datetime.now().strftime('%Y-%m-%d'), user_aim = str(), imt = float(), imt_str=str(),cal=float(),user_sex=str(),user_height=int(),user_weight=float(), user_age=int() )
...
def log_in(message):
  ...
  if cursor.fetchone():
    ...
    btn = 'Регистрация'
    markup.add(btn)
    bot.send_message(message.chat.id, text = 'Твоих данных в базе нету :( Для начала пройди регистрацию', reply_markup=markup)
