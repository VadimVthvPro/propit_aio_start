...
async def leng(message: Message, state: FSMContext):
    ...
    await message.answer(text = 'Please, chose your lenguage:', reply_markup=kb.lenguage)

@dp.message(REG.leng)
async def start(message: Message):
    async with aiosqlite.connect('pro3.db') as conn:
        await conn.execute(
            'INSERT INTO user_lang (user_id, leng) VALUES (?, ?)',
            (message.from_user.id, lenguages[message.text])
        )
        await conn.commit()
    await message.answer_photo(
        FSInputFile(path='new_logo.jpg'),
        caption= l.printer(message.from_user.id, "start").format(message.from_user.id),
...
async def entrance(message: Message, state: FSMContext):
    ...
    if user_data:
        ...
        await bot.send_message(message.chat.id, text=l.printer(message.from_user.id, 'MissedReg'), reply_markup=kb.reRig)
...
async def age(message: Message, state: FSMContext):
    ...
    await message.answer(l.printer(message.from_user.id, 'sex'), reply_markup=kb.sex)

@dp.message(REG.sex)
async def sex(message: Message, state: FSMContext):
    await state.update_data(sex=message.text)
    await state.set_state(REG.want)
    await message.answer(l.printer(message.from_user.id, 'aim'), reply_markup=kb.want)
...
async def wei(message: Message, state: FSMContext):
    ...
    await message.answer(l.printer(message.from_user.id, 'SuccesfulRig'), reply_markup=kb.main_menu)
    await state.clear()


async def calculate_imt_description(imt, message:Message):
    if round(imt) < 15:
...
async def calculate_calories(sex, weight, height, age, message):
...
async def tren(message: Message, state: FSMContext):
    await bot.send_message(message.chat.id, text=l.printer(message.from_user.id, 'TrenType'), reply_markup=kb.tren)
...
async def food1(message: Message, state: FSMContext):
    await message.answer(text=l.printer(message.from_user.id, 'ChooseTheWay'), reply_markup=kb.food)
...
async def chat(message:Message):
    ...
    'INSERT INTO water (user_id, date, count) VALUES (?, ?, ?)',
        ...
        await conn.commit()
    await message.answer(text = l.printer(message.from_user.id, 'cup'), reply_markup=kb.main_menu)
        ...
        with open('products.json') as f:
            ...
            for i in range(len(foods)):
                if foods[i]["name"] == name_a[m].title():
                    ...
                    conn.commit()
    await bot.send_message(message.chat.id, text=l.printer(message.from_user.id, 'InfoInBase'), reply_markup=kb.main_menu)
    await state.clear()
...
async def ai(message: Message, state: FSMContext):
    ...
    try:
        if plan_pit and plan_train:
            ...
            await bot.send_message(message.chat.id, text="Not enough info about user", reply_markup=kb.main_menu)
    except Exception as e:
        print(f"Ошибка при генерации плана: {str(e)}")
        return f"Ошибка при генерации плана: {e}"
...
async def ai_food(message: Message, state: FSMContext):
    await message.answer(text = l.printer(message.from_user.first_name, 'choosemeal'), reply_markup=kb.meals)
...
async def ai_food(message: Message, state: FSMContext):
    await message.answer(text = l.printer(message.from_user.id, 'trenchoose'), reply_markup=kb.tren_type)
...
async def choising(message: Message, state: FSMContext):
    ...
    if mes == l.printer(message.from_user.id, 'return'):
        await message.answer(text = l.printer(message.from_user.id, 'ok'),reply_markup=kb.main_menu)
    else:
        await message.answer(text = l.printer(message.from_user.id, 'unhappy'), reply_markup=kb.main_menu)

@dp.message(F.text == 'Вход в програму' or F.text == 'Entrance')
async def ais(message: Message, state: FSMContext):
    await message.answer( text=l.printer(message.from_user.id, 'begining'),reply_markup=kb.main_menu
)

@dp.message(F.text == 'Сводка')
async def svod(message: Message, state: FSMContext):
    await message.answer(text = l.printer(message.from_user.id, 'svoDAY'), reply_markup=kb.svo)
